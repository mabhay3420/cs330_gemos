...
/*
    XXX  You may or may not use any of these function. They either implement
   some callbacks for exit/mmap handling or provide some helpers. You *MUST NOT*
   modify the following implementation. IF YOU NEED AN ALTERNATE IMPLEMENTATION
   DEFINE IT IN clone_threads.c AND USE IT!
*/
...
// Find the thread private map meta-data for the given address. The address
// should be
//  an exact match. If length=0, length is not matched else it is matched with
//  the previously recored information. NOTE:  The range check is not done here.
//  You should implement your version in clone_threads.c to check if an address
//  is in range of a thread_private_map

static struct thread_private_map *find_thmap(struct thread *th, u64 address,
                                             u32 length) {
  int ctr;
  struct thread_private_map *thmap = &th->private_mappings[0];
  for (ctr = 0; ctr < MAX_PRIVATE_AREAS; ++ctr, thmap++) {
    if (thmap->owner && thmap->start_addr == address &&
        (!length || thmap->length == length))
      return thmap;
  }
  return NULL;
}
...
struct thread *find_thread_from_address(struct exec_context *ctx, u64 addr,
                                        u32 length,
                                        struct thread_private_map **thmap) {
  int i;
  struct thread *th;
...
long do_make_thread_ready(long pid) {
  ...
  if (ctx->type != EXEC_CTX_USER_TH || ctx->state == EXITING ||
      ctx->state == UNUSED)
    return -EINVAL;
  ctx->state = READY;
  return 0;
